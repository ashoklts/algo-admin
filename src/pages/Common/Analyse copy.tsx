  ...
  const fetchData = useCallback(() => {
    ...
    fetch(url)
      ...
      .then((data: Record<string, unknown>) => {
        ...
        const rawOpen: Record<string, unknown>[]   = ((data.legs as Record<string, unknown>)?.open  as Record<string, unknown>[]) ?? [];
        const rawClosed: Record<string, unknown>[] = ((data.legs as Record<string, unknown>)?.closed as Record<string, unknown>[]) ?? [];
        const toAnalyseLeg = (leg: Record<string, unknown>, exited: boolean) => {
          const et = (leg.entry_trade ?? {}) as Record<string, unknown>;
          const xt = (leg.exit_trade ?? {})  as Record<string, unknown>;
          const ot = String(et.option_type ?? leg.option_type ?? "CE").toUpperCase();
          return {
            token:      String(leg.token ?? et.instrument_token ?? "").trim(),
            entryDate:  String(leg.entry_timestamp ?? et.traded_timestamp ?? "").slice(0, 10) || new Date().toISOString().slice(0, 10),
            expiry:     String(et.expiry ?? leg.expiry ?? ""),
            optionType: (ot === "PE" ? "Put" : "Call") as "Call" | "Put",
            premium:    Number(leg.entry_price ?? et.price ?? 0),
            quantity:   Number(leg.effective_quantity ?? leg.quantity ?? 1),
            strike:     Number(et.strike ?? leg.strike_price ?? 0),
            type:       (String(leg.position_side ?? leg.position ?? "sell").toLowerCase() === "buy" ? "Buy" : "Sell") as "Buy" | "Sell",
            exited,
            exitPrice:  exited ? (Number(xt.price ?? xt.trigger_price ?? leg.exit_price ?? 0) || null) : null,
          };
        };
        const legs = [
          ...rawOpen.map(l => toAnalyseLeg(l, false)),
          ...rawClosed.map(l => toAnalyseLeg(l, true)),
        ].filter(l => l.strike > 0 && l.expiry);
        if (legs.length > 0) setOptionLegs(legs);
      })
      .catch(() => {});
  }, [type, id]);
