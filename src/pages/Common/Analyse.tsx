...
const styles = scopeCss(`
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .floating-pnl { position: absolute; top: 6px; background: #15803d; color: #fff; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; pointer-events: none; z-index: 11; white-space: nowrap; display: flex; align-items: center; gap: 5px; }
  .floating-pnl.loss { display: flex; top: -25px; background: #b91c1c; }
...
export default function SLSimulatorPage() {
  ...
  function calcZoomHalf(spotPrice: number): number {
    const oneSdMove = getOneSdMove(spotPrice);
    const twoSdMove = oneSdMove * 2;

    if (!Number.isFinite(twoSdMove) || twoSdMove <= 0) {
      return 1500;
    }

    return Math.max(500, Math.ceil(twoSdMove / 50) * 50);
  }
    ...
    <div className="sl-page" style={{ margin: "-24px", padding: "24px" }}>
      ...
      <div className="main-container">
        <div className="option-chain-panel">
            ...
            <div className="simulator-stats">
              ...
              <div>
                ...
                <div className="sim-stat-value simulator_green_text">
                  {isUnlimitedProfit
                    ? <span>Unlimited</span>
                    : <><span>{fmtCurrency(maxProfitValue, 2)}</span><span className="metrics-sub simulator_green_text">({maxProfitPct.toFixed(1)}%)</span></>
                  }
                </div>
              </div>
              <div>
                ...
                <div className="sim-stat-value simulator_red_text">
                  {isUnlimitedLoss
                    ? <span>Unlimited</span>
                    : <><span>{fmtCurrency(maxLossValue, 2)}</span><span className="metrics-sub simulator_red_text">({maxLossPct.toFixed(1)}%)</span></>
                  }
                </div>
              </div>
              <div>
          ...
          {spotVisible && (() => {
            const pct = currentSpotPrice > 0 ? ((payoffAtSpot / currentSpotPrice) * 100).toFixed(3) : "0.000";
            ...
            return (
              <div style={{ position: "relative", height: 10 }}>
                <div
                  ...
                  style={{ left: (spotX || 0) - badgeWidth / 2 }}
                >
