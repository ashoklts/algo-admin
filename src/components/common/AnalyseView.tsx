...
interface Leg {
  ...
  strike: number;
  type: TradeType;
  apiPnl?: number;        // pre-computed PnL from backend (leg.pnl)
  apiLtp?: number | null; // last_saw_price / mark_price from backend
}
...
const styles = scopeCss(`
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Option Chain Panel ── */
  .option-chain-panel { width: 760px; min-width: min(760px, 100%); flex: 0 0 auto; background: #fff; border-radius: 8px; border: 1px solid #e5e7eb; box-shadow: 0 2px 10px rgba(0,0,0,.08); overflow: hidden; display: flex; flex-direction: column; height: calc(100vh - 48px); }
  ...
  .chart-section { flex: 1 1 420px; min-width: 420px; width: 100%; background: #fff; padding: 25px; border-radius: 8px; box-shadow: 0 2px 10px rgba(0,0,0,.08); }
  ...
  .chart-wrapper { position: relative; height: 690px; margin-bottom: 20px; }
  .chart-wrapper canvas { display: block; width: 100% !important; height: 100% !important; }
  .payoff-tooltip { position: absolute; background: #1f2937; color: #f9fafb; border-radius: 8px; padding: 10px 14px; font-size: 12px; pointer-events: none; z-index: 20; min-width: 190px; box-shadow: 0 4px 20px rgba(0,0,0,.4); border: 1px solid rgba(255,255,255,.1); }
  .payoff-tooltip-title { font-size: 10px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: .5px; margin-bottom: 7px; padding-bottom: 6px; border-bottom: 1px solid rgba(255,255,255,.1); }
  .payoff-tooltip-spot-label { font-size: 10px; color: #a1a1aa; margin-bottom: 3px; }
  .payoff-tooltip-spot-price { display: flex; align-items: baseline; gap: 6px; margin-bottom: 9px; flex-wrap: wrap; }
  .payoff-tooltip-spot-main { font-size: 18px; font-weight: 700; line-height: 1.05; color: #fff; }
  .payoff-tooltip-spot-diff { font-size: 11px; font-weight: 700; line-height: 1; }
  .payoff-tooltip-spot-diff.up { color: #22c55e; }
  .payoff-tooltip-spot-diff.down { color: #f87171; }
  .payoff-tooltip-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 3px; gap: 10px; }
  .payoff-tooltip-label { color: #9ca3af; font-size: 11px; }
  .payoff-tooltip-value { font-weight: 600; font-size: 12px; text-align: right; }
  .payoff-tooltip-divider { margin-top: 5px; padding-top: 5px; border-top: 1px solid rgba(255,255,255,.08); }
  .spot-line { position: absolute; width: 1px; pointer-events: none; z-index: 10; }
  ...
  .sd-guide-line { position: absolute; width: 0; pointer-events: none; z-index: 9; border-left: 1px dashed rgba(148, 163, 184, 0.9); }
  .sd-guide-label { position: absolute; top: 6px; transform: translateX(-50%); font-size: 11px; color: #64748b; font-weight: 600; white-space: nowrap; }
  ...
  .floating-pnl { position: absolute; background: #15803d; color: #fff; padding: 5px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; pointer-events: none; z-index: 20; white-space: nowrap; display: flex; align-items: center; gap: 5px; }
  .floating-pnl.loss { background: #b91c1c; }
  ...
  .tooltip-hover-line { position: absolute; width: 0; pointer-events: none; z-index: 11; border-left: 1px dashed rgba(31, 41, 55, 0.75); }
  ...
  /* ── When chart wraps below (container < 760+420+20=1200px), cap panel height ── */
  ...
  @media (max-width: 640px) {
    ...
    .simulator-stats { flex-wrap: wrap; }
    .simulator-stats > div { min-width: 50%; border-right: none !important; }
  }
`, ".sl-page");
...
function calcImpliedIV(
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const price = optionType === "Call" ? blackScholesCall(S, K, T, r, mid) : blackScholesPut(S, K, T, r, mid);
...
export default function AnalyseView({
  ...
  const originalLegsRef = useRef<Leg[]>([]);
  const optionLegsRef = useRef<Leg[]>([]);
  ...
  const [leftTab, setLeftTab] = useState<"Positions" | "OptionChain">("Positions");
  ...
  function recalcMarginLocal(legs: Leg[], underlying: string, spotPrice: number, liveMap: Record<string, number> = {}) {
    ...
    const positions: SpanPos[] = legs.map(leg => ({
      underlying,
      instrument_type: leg.optionType === "Call" ? "CE" : "PE",
      expiry: leg.expiry,
      strike: leg.strike,
      transaction_type: leg.type === "Buy" ? "BUY" : "SELL",
      quantity: leg.lotSize && leg.lotSize > 0 ? Math.round(leg.quantity / leg.lotSize) : leg.quantity,
      lot_size: leg.lotSize || 1,
      ltp: (leg.token && liveMap[leg.token] != null) ? liveMap[leg.token] : (leg.liveLtp ?? leg.premium ?? 0),
      spot: spotPrice || 0,
    }));
    console.log("[MARGIN][AnalyseView] positions:", JSON.parse(JSON.stringify(positions)));
    const result = calculateMargin(positions);
    console.log("[MARGIN][AnalyseView] result:", JSON.parse(JSON.stringify(result)));
    const net = result.net_margin > 0 ? result.net_margin : result.total_margin;
    if (net > 0) setSummaryMeta(prev => ({ ...prev, marginBlocked: net }));
  }
  ...
  const fetchData = useCallback(() => {
    ...
    fetch(url)
      ...
      .then((data: Record<string, unknown>) => {
        ...
        setSummaryMeta({ marginBlocked: null }); // will be updated by /algo/margin/calculate below
        ...
        const legs = [...rawOpen, ...rawQueued, ...rawClosed]
          ...
          .filter((leg) => leg.strike > 0 && leg.expiry);
        setOptionLegs(legs);
        originalLegsRef.current = legs;

        // Calculate margin locally using spanMargin.ts — triggered by execute_order socket or initial load
        const effectiveUnderlying = ul || underlyingRef.current;
        const openForMargin = legs.filter(l => !l.exited && !l.isQueued && l.strike > 0 && l.expiry);
        if (openForMargin.length && effectiveUnderlying) {
          const liveSpot = spot > 0 ? spot : currentSpotPriceRef.current;
  ...
  // keep refs in sync so socket handlers can read latest values without stale closure
  useEffect(() => { optionLegsRef.current = optionLegs; }, [optionLegs]);
  useEffect(() => { tokenLtpMapRef.current = tokenLtpMap; }, [tokenLtpMap]);

  // ── Replay: filter legs by timestamp ────────────────────────────────────────
  useEffect(() => {
    if (!replayTimestamp) {
      if (originalLegsRef.current.length) setOptionLegs(originalLegsRef.current);
      return;
    }
    const replayTs = new Date(replayTimestamp.replace(" ", "T"));
    const filtered = originalLegsRef.current
      .filter(leg => {
        const entryDt = new Date((leg.entryDate || "").replace(" ", "T"));
        return !isNaN(entryDt.getTime()) ? entryDt <= replayTs : true;
      })
      .map(leg => {
        const exitDt = leg.exitDate ? new Date(leg.exitDate.replace(" ", "T")) : null;
        if (leg.exited && exitDt && exitDt > replayTs) {
          return { ...leg, exited: false, exitPrice: null, exitDate: "", exitDelta: null };
        }
        return leg;
      });
    setOptionLegs(filtered);
  }, [replayTimestamp]);

  // ── Replay: override LTP map ────────────────────────────────────────────────
  useEffect(() => {
    if (replayLtpMap && Object.keys(replayLtpMap).length > 0) {
      setTokenLtpMap(replayLtpMap);
    }
  }, [replayLtpMap]);
  ...
  // ── Replay: spot price drives currentSpotPrice (spot line + IV calc) ────────
  useEffect(() => {
    if (replaySpotPrice && replaySpotPrice > 0) {
      setCurrentSpotPrice(replaySpotPrice);
      setUserCustomZoom(false);
    }
  }, [replaySpotPrice]);
    ...
    function connectExOrd() {
      ...
      ws.onmessage = e => {
        ...
        if (p.type === "execute_order") {
          ...
          // recalc margin after 500ms so latest LTP prices are already in tokenLtpMapRef
          if (marginTimerRef.current) clearTimeout(marginTimerRef.current);
          marginTimerRef.current = setTimeout(() => {
            const openLegs = optionLegsRef.current.filter(l => !l.exited && !l.isQueued && l.strike > 0 && l.expiry);
            if (!openLegs.length || !underlyingRef.current) return;
            recalcMarginLocal(openLegs, underlyingRef.current, currentSpotPriceRef.current, tokenLtpMapRef.current);
          }, 500);
        }
      };
  ...
  useEffect(() => {
    ...
    async function pollOptionChain() {
      ...
      try {
        ...
        // Subsequent polls must not touch spot price — that would trigger a
        // full payoff chart re-render via currentSpotPrice in chart useEffect deps.
        const spotFromResp = Number(json.spot_price ?? 0);
        if (spotFromResp > 0 && !currentSpotPriceRef.current) {
          setCurrentSpotPrice(spotFromResp);
        }
        // selectedExpiry reset is handled by the expiries useEffect below
      } catch {
        // silent — polling failure must not affect position socket loop
      }
    }
  ...
  function getLegIV(leg: Leg): number {
  ...
  function calcZoomRange(spotPrice: number): { start: number; end: number } {
    const legs = optionLegs.filter(l => !l.isQueued && !l.exited && l.strike > 0);

    if (!legs.length) {
      // No legs yet: ±3% of spot as a neutral default
      const half = Math.max(300, Math.ceil(spotPrice * 0.03 / 50) * 50);
      return {
    ...
    const lo = Math.min(...strikes, spotPrice);
    const hi = Math.max(...strikes, spotPrice);
    const strategyWidth = hi - lo;

    // padding = max(3% of spot, 40% of strategy width, 200 pts)
    const padding = Math.max(
      Math.ceil(spotPrice * 0.03 / 50) * 50,
      Math.ceil(strategyWidth * 0.40 / 50) * 50,
      200
    );
  ...
  // Sensibull "Today" P&L:
  //   - Exited legs  → realized P&L (fixed; does NOT vary as spot moves on x-axis)
  //   - Open legs    → Black-Scholes with T = now→expiry and per-strike IV implied
  //                    from live LTP (or chain IV / entry IV as fallback)
  function calculateCurrentDayPnL(spotPrice: number): number {
    ...
    return optionLegs.reduce((total, leg) => {
      ...
      if (leg.exited) {
        const realizedPnl = leg.type === "Buy"
          ? ((leg.exitPrice ?? leg.premium) - leg.premium) * leg.quantity
          : (leg.premium - (leg.exitPrice ?? leg.premium)) * leg.quantity;
      ...
      const sigma =
        liveLtp && liveLtp > 0
          ...
          : getLegIV(leg);
      const optionValue = leg.optionType === "Call"
        ? blackScholesCall(spotPrice, leg.strike, T, r, sigma)
        : blackScholesPut(spotPrice, leg.strike, T, r, sigma);
      const pnl = leg.type === "Buy"
  ...
  function getOneSdMove(spotPrice: number, expiry?: string): number {
    const activeExpiry = expiry || selectedExpiry || expiries[0];
    ...
    const sigma = Math.max(0.01, getEffectiveIV(activeExpiry));
    const oneSdMove = spotPrice * sigma * Math.sqrt(T);
    return Number.isFinite(oneSdMove) ? oneSdMove : 0;
  }
  ...
  useEffect(() => {
    ...
    // Reset if not set OR if current value no longer exists in the new expiry list
  ...
  useEffect(() => {
    if (!zoomStartPrice && !zoomEndPrice) {
      const range = calcZoomRange(currentSpotPrice);
      setZoomStartPrice(range.start);
      setZoomEndPrice(range.end);
    }
  }, [optionLegs, currentSpotPrice, zoomEndPrice, zoomStartPrice]);
  ...
  }, [currentSpotPrice, optionLegs, selectedExpiry, userCustomZoom]);

  // frozenAdjLevels is only set when user explicitly requests adjustment — not auto-triggered
  useEffect(() => {
  ...
  }, [currentMarketTime, currentSpotPrice, ivShiftPct, optionLegs, riskFreeRate, selectedExpiry, tokenLtpMap, zoomEndPrice, zoomStartPrice]);

  // OI-only update — only updates OI bar datasets[0,1] with no animation.
  // Does NOT touch payoff line datasets[2,3] → payoff chart visually unchanged.
  ...
  useEffect(() => {
    ...
    (chart.options.scales as any).yOI.min = -oiAxisMax;
    (chart.options.scales as any).yOI.max = oiAxisMax;
    chart.update("none");
  }, [optionChainData, selectedExpiry]);
  ...
  function getLegLTP(leg: Leg): number {
    ...
    if (tok && replayLtpMap) {
      const v = replayLtpMap[tok] ?? replayLtpMap[`NSE_${tok}`] ?? replayLtpMap[tok.replace(/^NSE_/i, "")];
      if (v != null) return v;
    }
    if (tok && tokenLtpMap[tok] != null) return tokenLtpMap[tok];
    if (leg.liveLtp != null) return leg.liveLtp;
    if (leg.apiLtp != null) return leg.apiLtp;
  ...
  function getLegPnl(leg: Leg): number {
    ...
    if (leg.exited) {
      // Use API's pre-computed PnL for closed legs — matches Execution page exactly
      if (leg.apiPnl !== undefined && leg.apiPnl !== 0) return leg.apiPnl;
      return leg.type === "Buy"
        ? ((leg.exitPrice ?? leg.premium) - leg.premium) * leg.quantity
        : (leg.premium - (leg.exitPrice ?? leg.premium)) * leg.quantity;
    }
    // Open leg: use live LTP from socket; fall back to apiPnl when no live data
    const tok = leg.token?.trim();
    const hasLiveLtp = (tok && tokenLtpMap[tok] != null) || leg.liveLtp != null || leg.apiLtp != null;
    if (!hasLiveLtp && leg.apiPnl !== undefined) return leg.apiPnl;
  ...
  const maxProfitPct = marginBasis ? (maxProfitValue / marginBasis) * 100 : 0;
  const maxLossPct = marginBasis ? (maxLossValue / marginBasis) * 100 : 0;
  const netCreditValue = calcNetCredit();
  const popValue = calcPOP();
  const breakevenPoints = calcBreakevens();
  // Unlimited profit/loss: check slope far beyond all strikes
  const activeOpenLegs = optionLegs.filter(l => !l.isQueued && l.includeInPnl !== false && !l.exited);
  const allStrikes = activeOpenLegs.map(l => l.strike);
  const topStrike = allStrikes.length ? Math.max(...allStrikes) : currentSpotPrice;
  const bottomStrike = allStrikes.length ? Math.min(...allStrikes) : currentSpotPrice;
  const pnlFarUp1 = calculatePnL(topStrike * 2);
  const pnlFarUp2 = calculatePnL(topStrike * 3);
  const pnlFarDown1 = calculatePnL(bottomStrike * 0.5);
  const pnlFarDown2 = calculatePnL(bottomStrike * 0.1);
  const isUnlimitedProfit = (pnlFarUp2 - pnlFarUp1) > 1;
  const isUnlimitedLoss = (pnlFarUp2 - pnlFarUp1) < -1 || (pnlFarDown2 - pnlFarDown1) < -1;
  ...
  const sdExpiry = nearestLegExpiry;
  ...
  // Compare only first 10 chars so "2025-11-04 15:30:00" matches "2025-11-04"
  const selExp10 = (selectedExpiry || "").slice(0, 10);

  function hasOcPos(strike: number, optionType: "Call" | "Put", type: "Buy" | "Sell"): boolean {
    return optionLegs.some(
      (l) => !l.isQueued && !l.exited
        && l.expiry.slice(0, 10) === selExp10
        && l.strike === strike
        && l.optionType === optionType
        && l.type === type
    );
  }

  function countOcPos(strike: number, optionType: "Call" | "Put", type: "Buy" | "Sell"): number {
    return optionLegs.filter(
      (l) => !l.isQueued && !l.exited
        && l.expiry.slice(0, 10) === selExp10
        && l.strike === strike
        && l.optionType === optionType
  ...
  function toggleOcPos(strike: number, optionType: "Call" | "Put", type: "Buy" | "Sell", premium: number) {
    setOptionLegs((prev) => {
      ...
      // Infer lot size from existing legs on the same expiry, else default 1
      const refLeg = prev.find(l => l.expiry.slice(0, 10) === selExp10 && (l.lotSize ?? 0) > 0);
      const lotSize = refLeg?.lotSize ?? 1;
      const quantity = lotSize * multiplier;

      const ocType = optionType === "Call" ? "CE" : "PE";
      const chainRow = optionChainData.find(
        r => r.expiry.slice(0, 10) === selExp10 && r.strike === strike && r.type === ocType
      );

      return [
        ...prev,
        {
          entryDate: currentMarketTime.toISOString(),
          ...
          lotSize,
          optionType,
          premium: premium || chainRow?.close || 0,
          quantity,
          strike,
          type,
          entryIv: chainRow?.iv ?? null,
        } as Leg,
      ];
    });
  }
            ...
            <div className="simulator-stats">
              ...
              <div>
                ...
                <div className="sim-stat-value simulator_green_text">
                  {isUnlimitedProfit
                    ? <span>Unlimited</span>
                    : <><span>{fmtCurrency(maxProfitValue, 2)}</span><span className="metrics-sub simulator_green_text">({maxProfitPct.toFixed(2)}%)</span></>
                  }
                </div>
              </div>
              <div>
                ...
                <div className="sim-stat-value simulator_red_text">
                  {isUnlimitedLoss
                    ? <span>Unlimited</span>
                    : <><span>{fmtCurrency(maxLossValue, 2)}</span><span className="metrics-sub simulator_red_text">({maxLossPct.toFixed(2)}%)</span></>
                  }
                </div>
              </div>
              <div>
